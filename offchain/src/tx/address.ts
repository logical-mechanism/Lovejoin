...
// optionally staked.
//
// Spec: CIP-19 (Cardano addresses), BIP-173 (bech32). Lovejoin parks all
// dApp UTxOs at *base* script addresses with a constant per-network
// stake-key credential (the dApp's "stake key"). Every mix-box and fee
// shard generated by the SDK is therefore delegated to whatever pool the
// dApp's stake key chose, accruing rewards for the dApp operators while
// custody stays with the validators (the payment credential is a script
// hash, not a key, so users still can't be impersonated).
//
// For backwards compatibility with the existing Preprod bootstrap (which
// produced enterprise addresses), the stake hash is optional — when
// absent the builder emits an enterprise address. Re-bootstrapping would
// flip the live deployment over.
//
...
// test harness.
//
// Address formats (CIP-19):
//   Enterprise (payment = script, no staking):
//     header   = 0b0111 0000 | networkId
//     payload  = header (1) || scriptHash (28)             → 29 bytes
//   Base (payment = script, stake = key):
//     header   = 0b0001 0000 | networkId
//     payload  = header (1) || scriptHash (28) || stakeKeyHash (28) → 57 bytes
...
const BASE_SCRIPT_KEY_HEADER_HIGH_NIBBLE = 0x10; // 0b0001_0000

const BECH32_CHARSET = "qpzry9x8gf2tvdw0s3jn54khce6mua7l";

/**
 * Build a script address. When `stakeKeyHashHex` is provided the result is
 * a *base* address with the script payment credential and a stake-key
 * credential pointing at that hash; otherwise it's an enterprise address
 * (payment-only).
 *
 * Validators only inspect `payment_credential`, so adding a stake key
 * never changes whether a UTxO satisfies on-chain rules — it only
 * affects whether it earns staking rewards and counts toward delegation.
 *
 * @param scriptHashHex 28-byte payment script hash, lowercase hex.
 * @param networkId 0 for testnet (preprod/preview), 1 for mainnet.
 * @param stakeKeyHashHex Optional 28-byte stake-key hash. When set, builds
 *   a base address; otherwise an enterprise address.
 */
...
/**
 * @deprecated Use `buildScriptAddress(scriptHashHex, networkId)` instead —
 * this name kept as a thin alias to avoid churning callers in one go.
 */
export function buildEnterpriseScriptAddress(scriptHashHex: string, networkId: 0 | 1): string {
  return buildScriptAddress(scriptHashHex, networkId, null);
}
