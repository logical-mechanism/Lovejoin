// Cardano address construction — bech32 encoding for enterprise script
// addresses.
//
// Spec: CIP-19 (Cardano addresses), BIP-173 (bech32). The Lovejoin scripts
// are all parked at *enterprise* (payment-only, no staking) script
// addresses — this matches what the bootstrap ceremony produced via
// `cardano-cli address build --payment-script-file`. Computing them
// off-chain from the script hashes lets the SDK avoid eagerly importing
// mesh's CSL bindings, which fail to load under the test harness.
//
// Format (CIP-19):
//   header = 0b0111 0000 | networkId
//     0b0111 = address type: enterprise, payment-script credential
//     networkId: 0 = testnet, 1 = mainnet
//   payload = header (1 byte) || scriptHash (28 bytes)
...
/**
 * Build an enterprise script address (payment = script, no staking part).
 *
 * @param scriptHashHex 28-byte script hash, lowercase hex.
 * @param networkId 0 for testnet (preprod/preview), 1 for mainnet.
 */
export function buildEnterpriseScriptAddress(
  ...
  if (scriptHash.length !== 28) {
    throw new Error(
      `enterprise script address: script hash must be 28 bytes, got ${scriptHash.length}`,
    );
  }
