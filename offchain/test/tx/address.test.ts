...
//
// The protocol pins every continuing UTxO to a CIP-19 enterprise script
// address (payment = script hash, no stake credential, no reference
// script) — see contracts/lib/lovejoin/mixbox.ak `output_at_script`. The
// SDK's `buildScriptAddress` is therefore enterprise-only; there is no
// API for base / pointer / stake-script-key shapes.

import { describe, expect, it } from "vitest";

import { buildScriptAddress } from "../../src/tx/address.js";

describe("tx/address — buildScriptAddress", () => {
  it("encodes a preprod fee_contract enterprise address", () => {
    ...
    expect(buildScriptAddress(scriptHash, 0)).toBe(expected);
  });
  ...
  it("uses the mainnet HRP when networkId=1", () => {
    const scriptHash = "5efd8fdd7e4d35b04de427337220dcb30352136d739055b305dd2d66";
    const out = buildScriptAddress(scriptHash, 1);
    expect(out.startsWith("addr1")).toBe(true);
  });

  it("rejects script hashes with the wrong byte length", () => {
    expect(() => buildScriptAddress("ab".repeat(20), 0)).toThrow(/28 bytes/);
    expect(() => buildScriptAddress("ab".repeat(32), 0)).toThrow(/28 bytes/);
  });

  it("tolerates 0x prefix on input", () => {
    const scriptHash = "0x5efd8fdd7e4d35b04de427337220dcb30352136d739055b305dd2d66";
    expect(buildScriptAddress(scriptHash, 0)).toBe(
