...
function emitAikenTest(v: Vector, idx: number): string {
  const stmts = v.statements
    .map(
      (s) =>
        `      sigma_or.DHTupleStatement { ap: ${aikenLit(s.ap)}, bp: ${aikenLit(s.bp)} },`,
    )
    .join("\n");
...
function main() {
  ...
  const positives = aikenVecs
    .map((v, i) => emitAikenTest(v, i % AIKEN_PER_N))
    .join("\n\n");
  const negatives = aikenVecs
    .map((v, i) => emitAikenNegativeC(v, i % AIKEN_PER_N))
    .join("\n\n");
