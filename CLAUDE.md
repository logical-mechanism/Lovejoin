...
Three operations: **Deposit** (locks fixed-denom ADA into a mix-box, replenishes a fee shard), **Mix** (re-randomizes N pool boxes for `2 ≤ N ≤ max_n` via N-way sigma-OR proof, fully wallet-anonymous), **Withdraw** (Schnorr-proof spend by the original depositor). Privacy: `(1/N)^k` linkage probability after k mixes at width N. The UI composes these into higher-level user actions ("intensity dial", "fan-out") via the strategy layer in [offchain/src/strategy/](offchain/src/strategy/); see the "Strategy layer" section below.
...
1. **Hyperstructure via reference UTxO.** A single permanent UTxO at the always-False [`reference_holder`](contracts/validators/reference_holder.ak) validator holds the protocol NFT and a `ProtocolParams` inline datum (`denom_lovelace`, `max_fee_per_mix_lovelace`, three script hashes). All validators read parameters from `tx.reference_inputs` at spend time. Bootstrap is a one-shot, irreversible ceremony per network; `one_shot_mint` is parameterized by the `reference_holder` script hash so the NFT can only land at the right address with a sane `ReferenceDatum` (positive `denom_lovelace` and `max_fee_per_mix_lovelace`), enforced at mint time (audit L-01 / L-02). `max_n` is not on-chain; `mix_logic` enforces `N ≥ 2` as a general floor, and the upper cap is a tx-construction concern surfaced through `network.<net>.json`. `fee_shard_target` (=10) is similarly off-chain coordination, kept on-chain today (planned removal is tracked but deferred).
...
3. **Mandatory collateral provider for Mix txs.** Mix txs have **no submitter wallet input or signature**. Cardano still requires a collateral input (key-witnessed); it comes from an external service ([giveme.my](https://giveme.my/) by default, pluggable via the `CollateralProvider` interface in [offchain/src/tx/collateral.ts](offchain/src/tx/collateral.ts)). If the provider is unreachable, Mix submission is **blocked**. There is no fallback to wallet-collateral, because that would defeat wallet anonymity. The provider client retries transient HTTP failures with backoff and forwards `additional_utxos` for in-flight chaining; pinned giveme.my coordinates (pkh, vkey, collateral UTxOs) live in [offchain/src/tx/known-collateral-hosts.ts](offchain/src/tx/known-collateral-hosts.ts). Deposit and Withdraw use `WalletProvider` today; making `GivemeMyProvider` the default for those too is deferred so fresh wallets do not need a 5-ADA collateral UTxO.

4. **Sharded fee contract.** A logical pool of exactly 10 fee UTxOs at the [`fee_contract`](contracts/validators/fee_contract.ak) script. Two redeemers: `PayMixFee` (consumed by a Mix tx, requires `fee_in.lovelace − fee_out.lovelace == tx.fee`, `tx.fee ≤ max_fee_per_mix_lovelace`, and **`N ≥ 3`** mix-script inputs (the audit-driven anonymity-set floor on fee-paying mixes; wallet-collateral N=2 mixes remain legal via `mix_logic`'s general floor)) and `Replenish` (top up by a Deposit, strict value increase). Both paths preserve shard count. SDK + backend pick shards uniformly at random for concurrency. The mempool-aware path also avoids shards already pending in flight, and the SDK can splice in-flight Replenish outputs back into the picker via `extraShards` so chained Deposit→Mix flows do not collide with themselves.
...
## Strategy layer and in-flight tx chaining

Per-tx builders in [offchain/src/tx/](offchain/src/tx/) are the primitive layer. [offchain/src/strategy/](offchain/src/strategy/) sits above them and composes multiple txs into a single user-facing privacy-amplification action; the UI binds to the strategy layer, not to raw tx builders. Current strategies:

- **Intensity dial (k chained Mixes).** The unified `MixPanel` surface exposes a single dial whose `k` chains k Mix txs back-to-back; k=1 is labeled "Random". Each subsequent Mix consumes the previous Mix's outputs while they are still in the mempool via the `chainFrom` hook on the Mix builder, so the user does not have to wait for chain confirmation between steps.
- **Fan-out** ([offchain/src/strategy/fanout.ts](offchain/src/strategy/fanout.ts) + [orchestrator.ts](offchain/src/strategy/orchestrator.ts)). Planner picks N fresh inputs (owned boxes are eligible) per slot, splits work across slots, and the orchestrator submits txs in parallel while tracking in-flight UTxO refs and fee shards so concurrent submissions do not collide. The UI surface lives on the Vault page; it logs per-slot wall-clock timings and surfaces a slow-slot hint that names the actual cause (collateral retry, etc.) rather than blaming the network.

Three SDK mechanisms make this work and are easy to break:

1. **`chainFrom` on the Mix builder** ([offchain/src/tx/mix.ts](offchain/src/tx/mix.ts)). Caller passes the in-flight parent's outputs (Deposit, Replenish, or another Mix); the builder feeds them to mesh's internal fetcher so coin selection can resolve refs that aren't on chain yet. The in-flight outputs MUST also be referenced in `args.inputs` (or `args.feeShard` for a chained Replenish→Mix).
2. **`evaluateTxWithAdditionalUtxos` on the mesh sibling.** Optional sibling on `BlockfrostProvider` and `BackendMeshProvider` (NOT on the core `ChainProvider` interface; it lives on the `meshProvider()` returned by each implementation). Routes `additionalUtxoSet` to ogmios v6's `/utils/txs/evaluate/utxos?version=6` (Blockfrost) or the backend's `/evaluate` (which proxies to ogmios). Without this, script-cost evaluation fails when a tx spends an in-flight output.
3. **`additional_utxos` forwarded to giveme.my.** `GivemeMyProvider.signTxBody` accepts an `additional_utxos` opt and forwards it so the collateral provider's own evaluation sees the in-flight parent. `AdditionalUtxo` is a flat object, NOT a `[txin, txout]` tuple; that schema was wrong and corrected, and `c72a838` made the backend match. Don't fall back to Blockfrost on a 4xx from the backend (`47084d5`); those are caller errors that Blockfrost would also reject and the fallback hides the real bug.

When a Mix build fails because its fee shard was raced, the builder retries with a fresh shard (`473a65e`); when it retries with fee bumps, `actualFeeLovelace` tracks the final fee (not the plan-time cap) so the in-flight fee-shard tracker stays accurate.
...
- **Every dApp UTxO is at a CIP-19 enterprise script address** (payment = script hash, no stake credential, no attached reference script). The on-chain perimeter (`output_at_script` / `output_at_fee`) pins `stake_credential == None` and `reference_script == None` on every continuing protocol output (audit H-01 / M-01). Choosing `None` over a fixed dApp stake key means no party can ever earn rewards from the pool's principal and the pool can't be force-delegated to a dead pool. The indexer filters mix-boxes by the single canonical enterprise address per script.
...
Everything that talks to chain (SDK, UI, integration tests, stress tests, backend itself) goes through the [`ChainProvider`](offchain/src/chain/provider.ts) interface: `submitTx`, `getUtxos`, `getUtxoByRef`, `awaitConfirmation`, `getReferenceUtxo`, `getProtocolParameters`. Two implementations exist: [`BlockfrostProvider`](offchain/src/chain/blockfrost.ts) (the default for SDK consumers) and [`BackendChainProvider`](offchain/src/chain/backend.ts) (the self-hosted indexer + db-sync, with Blockfrost fallback). They are runtime-swappable via `network.<net>.json`'s `provider` block. Mesh tx-building is wired through a `meshProvider()` sibling on each implementation so the same fetcher/submitter shape is available in both query and tx-builder paths; the sibling also carries the optional `evaluateTxWithAdditionalUtxos` capability (see "Strategy layer and in-flight tx chaining"). M5 plans to collapse the dual interface into mesh's `IFetcher + ISubmitter`.

Implication for new code: never call Blockfrost directly. Add capabilities to `ChainProvider` (or its mesh sibling, for tx-builder-only concerns) and let the backend grow a matching implementation.
...
Validator tests target 100% rule coverage. Every invariant enforced by a validator must have both a positive and a negative test in the validator's `*.test.ak` file; CI fails on coverage gaps. Hot-path redeemers have `aiken bench` baselines (`mix_logic` Mix at N=2/3/4, `fee_contract` PayMixFee + Replenish, `mix_box` spend, `one_shot_mint`). Stress tests live in [stress-tests/](stress-tests/) (max-n calibration, fee calibration, fuzz) and run against Preprod via Blockfrost; they update [docs/perf.md](docs/perf.md) and `config/network.preprod.json`. Integration tests in [integration-tests/](integration-tests/) cover deposit-withdraw round-trips, full-lifecycle, mix-n2, mix-at-max-n, fee-exhaustion, and chained Mix on an in-flight Deposit (the chainFrom path). UI E2E uses Playwright on Preprod ([ui/](ui/) `pnpm test:e2e`).
