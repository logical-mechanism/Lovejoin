...
## Build history (what shipped)

Preserved here for orientation. The crypto and validator stacks were built bottom-up; each layer depends on the one below.
...
- **Validators (M2, done):** types/helpers, `one_shot_mint`, `reference_holder`, `mix_logic` (withdraw-zero, runs once per tx), `mix_box` (cheap delegator), `fee_contract`, variable N, bootstrap scripts.
- **Mesh viability (M3, resolved):** mesh handles the unconventional Mix tx shape (no submitter wallet input, externally-supplied collateral, exact-fee constraint linking shard input to shard output via `tx.fee`). No switch to lucid-evolution. The decision and the workarounds we needed (forcing Blockfrost ogmios v6 for evaluation, `*TxInReference` taking scriptSize + scriptHash, REWARD vs WITHDRAW tagging) are recorded in [docs/perf.md](docs/perf.md).
- **Mix tx + UI (M3.5, M4, done):** wallet-derived vault, deposit/withdraw/mix flows, empirical N caps calibrated against the live validator.
- **Mix optimisation (M4.5, M4.6, done):** pre-uncompressing sigma-OR statements in `mix_logic` (drops N² uncompresses to N), redundant length checks fused, own_input lookup fused with the mix/fee count fold, redeemer-Mix gate hoisted in `fee_contract`. Approximately 50M CPU shaved at N=4. Redeploy on Preprod with new addresses.
- **Backend + indexer (M5, done):** Node + Fastify backed by ogmios chainsync and db-sync, exposing the same `ChainProvider` shape as `BlockfrostProvider`. Runtime-swappable via `network.<net>.json`. `BackendChainProvider` plus a sibling `meshProvider()` so the self-hosted indexer is a drop-in for Blockfrost in both query and tx-builder paths.
- **UI v1 + design pass (M6, M6.5, done):** Pool / Vault / Box / Protocol screens; per-row mix; mix-round counter; toast and error-handling pass; mobile LCP optimizations; visual rework; fee-shard floor handling; mix-selection hardening.
- **CI/CD (M7, done):** GitHub Actions for lint, test, build, release; tag-driven release; Dependabot. DigitalOcean App Platform deploy spec; Cloudflare Tunnel sidecar for ogmios + db-sync.
